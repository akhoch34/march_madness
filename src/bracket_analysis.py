...
class SimplePredictor:
    def __init__(self, teams_df, seeds_df, slots_df, predictions_df, elo_df=None, stats_df=None, current_season=2025):
    ...
    def generate_matchup_explanation(self, team1_id, team2_id, day_num=None):
        ...
        if seed_diff > 0 and favorite_seed_num < underdog_seed_num:
            ...
            if matchup_key in historical_matchups:
                ...
                explanation_parts.append(f"Historically, #{favorite_seed_num} seeds have won {hist_pct:.2f}% of games against #{underdog_seed_num} seeds in the tournament.")
        ...
        if self.stats_df is not None:
            ...
            if favorite_stats and underdog_stats:
                ...
                if 'OffEff' in favorite_stats and 'OffEff' in underdog_stats:
                    ...
                    if abs(off_diff) > 5:
                        if off_diff > 0:
                            explanation_parts.append(f"{favorite['name']} has a more efficient offense ({favorite_stats['OffEff']:.1f} vs {underdog_stats['OffEff']:.1f}).")
                        else:
                            explanation_parts.append(f"While {underdog['name']} actually has a more efficient offense ({underdog_stats['OffEff']:.1f} vs {favorite_stats['OffEff']:.1f}), other factors favor {favorite['name']}.")
                
                # Defensive efficiency comparison
                if 'DefEff' in favorite_stats and 'DefEff' in underdog_stats:
                    def_diff = underdog_stats['DefEff'] - favorite_stats['DefEff']  # Lower is better for defense
                    if abs(def_diff) > 5:
                        if def_diff > 0:
                            explanation_parts.append(f"{favorite['name']} has a stronger defense, allowing {favorite_stats['DefEff']:.1f} points per 100 possessions compared to {underdog_stats['DefEff']:.1f} for {underdog['name']}.")
                        else:
                            explanation_parts.append(f"Although {underdog['name']} has a better defensive efficiency ({underdog_stats['DefEff']:.1f} vs {favorite_stats['DefEff']:.1f}), other factors favor {favorite['name']}.")
                
                # 3-point shooting
                if '3P%' in favorite_stats and '3P%' in underdog_stats:
                    three_diff = favorite_stats['3P%'] - underdog_stats['3P%']
                    if abs(three_diff) > 0.03:  # 3% difference in 3PT shooting is significant
                        if three_diff > 0:
                            explanation_parts.append(f"{favorite['name']} has a significant advantage in 3-point shooting ({favorite_stats['3P%']*100:.2f}% vs {underdog_stats['3P%']*100:.2f}%).")
                        else:
                            explanation_parts.append(f"{underdog['name']} is the better 3-point shooting team ({underdog_stats['3P%']*100:.2f}% vs {favorite_stats['3P%']*100:.2f}%).")
