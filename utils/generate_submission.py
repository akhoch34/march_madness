"""
Generate March Madness submission using top-performing methods from historical eval.

Reads eval_results.csv to identify best methods, generates predictions for the target season,
and produces a combined M+W submission CSV plus bracket PNGs.
...
def generate_predictions(
    ...
    gender : str
        "M" or "W".
    data_dir : str
        Data directory for the target season (e.g. "data/2026").
    output_dir : str
...
def expand_to_sample_submission(
    ...
    """Expand a submission to cover every ID in SampleSubmissionStage2.csv.

    Rows already present in df keep their Pred value.
    Missing rows (non-tourney team pairings) are filled with 0.5.
    Returns df unchanged if no sample submission file is found.
    """
    sample_path = os.path.join(data_dir, "SampleSubmissionStage2.csv")
    if not os.path.exists(sample_path):
        print(f"  [warn] No SampleSubmissionStage2.csv found in {data_dir} — submission not expanded")
        return df

    sample = pd.read_csv(sample_path)[["ID"]]
    sample = sample[sample["ID"].str.startswith(f"{season}_")].copy()
    if sample.empty:
        print(f"  [warn] SampleSubmissionStage2.csv has no rows for season {season}")
        return df

    merged = sample.merge(df, on="ID", how="left")
    n_missing = merged["Pred"].isna().sum()
    if n_missing > 0:
        print(f"  [info] Filling {n_missing:,} non-tournament matchups with 0.5")
        merged["Pred"] = merged["Pred"].fillna(0.5)

    print(f"  [info] Expanded submission: {len(df):,} predicted → {len(merged):,} total rows")
    return merged[["ID", "Pred"]]


def generate_brackets(season: int, methods: list[str], output_dir: str, data_dir: str, skip_existing: bool = True):
    """Generate bracket PNGs for each method/gender using generate_brackets_from_submissions."""
    try:
        from utils.generate_brackets_from_submissions import run as run_brackets
        print(f"\nGenerating bracket PNGs for season {season}...")
        run_brackets(
            years=[season],
            genders=["M", "W"],
            methods=methods,
        ...
        print(f"[warn] Bracket generation failed: {e}")
        import traceback
        traceback.print_exc()
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate March Madness submission using top historical methods.",
    ...
    parser.add_argument(
        ...
        help="Data directory for the target season. Default: data/{season}.",
    )
    parser.add_argument(
        "--output-dir",
    ...
    parser.add_argument(
        "--mens-method", default=None,
        help=(
            "Specific method to use for men's predictions (overrides eval-results lookup). "
            "E.g. --mens-method modeh7"
        ),
    )
    parser.add_argument(
        "--womens-method", default=None,
        help=(
            "Specific method to use for women's predictions (overrides eval-results lookup). "
            "E.g. --womens-method xgb_ensemble_v2"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Skip bracket PNG generation.",
    )
    args = parser.parse_args()

    season = args.season
    data_dir = args.data_dir or os.path.join(PROJECT_ROOT, "data", str(season))
    if not os.path.isdir(data_dir):
        print(f"[error] Data directory not found: {data_dir}")
        sys.exit(1)

    # Manual method overrides: {gender: method_name}
    manual_methods = {}
    if args.mens_method:
        manual_methods["M"] = args.mens_method
    if args.womens_method:
        manual_methods["W"] = args.womens_method

    skip_existing = not args.no_skip_existing
    all_submissions = []
    all_methods_used = set()
    chosen = {}  # gender → method name used (for output filename)
    ...
    for gender in ("M", "W"):
        ...
        all_methods_used.update(preds.keys())
    ...
    # Expand to all required matchups (Stage 2 needs every team pairing, not just tourney teams)
    print("\nExpanding to full sample submission coverage...")
    final_sub = expand_to_sample_submission(final_sub, data_dir, season)
    final_sub = final_sub.sort_values("ID").reset_index(drop=True)

    # Name the file after the chosen methods when manually specified, otherwise use strategy
    if manual_methods:
    ...
    if not args.no_brackets:
        generate_brackets(
            season=season,
            methods=sorted(all_methods_used),
            output_dir=args.output_dir,
            data_dir=data_dir,
            skip_existing=skip_existing,
        )
